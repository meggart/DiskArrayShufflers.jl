module PickAxisArrays
export PickAxisArray

struct PickAxisArray{T,N,AT<:AbstractArray,P,PERM} <: AbstractArray{T,N}
    parent::AT
end
...
function PickAxisArray(parent, indmask, perm = nothing)
    ...
    im = (o...,)
    T = anycol(im) ? Array{eltype(parent),ncol(im)} : eltype(parent)
    nd = length(f)
    PickAxisArray{T,nd,typeof(parent),im,perm}(parent)
end
...
function Base.size(p::PickAxisArray)
    im = indmask(p)
    ii2 = filter(i->!isa(i,Union{Colon,Window}),im)
    permsize(p,map(i->size(p.parent,i),ii2))
end

permsize(::PickAxisArray{<:Any,<:Any,<:Any,<:Any,P}, x) where {P} = x[P]
permsize(::PickAxisArray{<:Any,<:Any,<:Any,<:Any,nothing}, x) = x
...
Base.view(p::PickAxisArray, i::CartesianIndex) = view(p,i.I...)
end